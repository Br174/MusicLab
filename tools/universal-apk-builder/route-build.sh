...
check_ready_artifact() {
  local reuse token manifest_path current_commit current_branch tmp http_code result first_line
  ...
  result=$(python3 - "$tmp" "$current_commit" "${UAB_GITHUB_REPOSITORY:-}" "$current_branch" <<'PY'
import base64, json, sys
path, commit, source_repo, branch = sys.argv[1:5]
try:
    ...
    sys.exit(2)
if data.get('status') != 'ready' or data.get('commit') != commit:
    sys.exit(3)
...
PY
    ...
    rm -f "$tmp"
    log "Artifact-first: nessun APK/ZIP pronto per il commit corrente; serve una build."
    return 1
  }
  rm -f "$tmp"

  first_line="$(printf '%s\n' "$result" | head -n1)"
  [[ "$first_line" == "READY" ]] || return 1

  log "Artifact-first: build già pronta per commit $current_commit. Nessuna ricompilazione."
